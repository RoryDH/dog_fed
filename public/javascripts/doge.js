...
var app = angular.module("dogFed", []);

app.controller("FeedController", function($scope, $http){
	var prepareResponse = function(res) {
		res.feeds.map(function(f){
			f.feedTime = new Date(f.feedTime);
			return f;
		});
		res.timeNow = new Date(res.timeNow);
		return res;
	}
	// -1: error,    0: Loading,    1: Loaded,    2: you sure,    3: sending,    4: done
	$scope.feedStatus = 0;
	$scope.errorMessage = "Unknowen error"
	//prettyDate(d.toISOString())
	$http({method: "GET", url:"/feeds.json"}).success(function(r, status, headers, config){
  	$scope.r = prepareResponse(r);
  	$scope.feedStatus = 1;
  }).error(function(r, status, headers, config){
  	$scope.feedStatus = -1;
  	$scope.errorMessage = "Error contacting server!"
  });
	$scope.newFeed = function(){
		if ($scope.feedStatus === 1) {
			$scope.feedStatus = 2;
			return;
		}
		$scope.feedStatus = 3;
	...
	$scope.prettyDate = function(time) {
		var date = new Date((time.toISOString() || "").replace(/-/g,"/").replace(/[TZ]/g," ")),
			diff = ( ($scope.r.timeNow.getTime() - date.getTime()) / 1000 ),
	...
	$scope.displayTime = function(tObj) {
		return strftime('%R on %a, %o of %h %Y', tObj);
	}
});
